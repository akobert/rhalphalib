...
#		self.Data_f9.Rebin2D(2,1)
	...
	#	ofile.WriteObject(self.TTBar_f9, "TTBar_fail_jet_pt_rho")
		...
		h1 = TH2F("ratio", "Ratio of Pass/Fail Events in Jet pT vs. Softdrop Mass", 40, 0, 2000, 40, 0, 200)

		for i in range(1, self.Data_p7.GetNbinsX()+1):
			for j in range(1, self.Data_p7.GetNbinsY()+1):
				rpass = self.Data_p7.GetBinContent(i,j) - self.TTBar_p7.GetBinContent(i,j)
				rfail = self.Data_f7.GetBinContent(i,j) - self.TTBar_f7.GetBinContent(i,j)
				if rpass<=0 or rfail<=0 or (rpass/rfail)>=10:
