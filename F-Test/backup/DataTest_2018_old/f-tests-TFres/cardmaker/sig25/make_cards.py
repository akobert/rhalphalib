...
#                    failTempl = get_template('GJ', 0, binindex+1, cat+'_', obs=msd, syst='nominal')
#                    passTempl = get_template('GJ', 1, binindex+1, cat+'_', obs=msd, syst='nominal')
...
#                    failCh = GJmodel['ptbin%dmjjbin%d%sfail%s' % (ptbin, mjjbin, cat, year)]
#                    passCh = GJmodel['ptbin%dmjjbin%d%spass%s' % (ptbin, mjjbin, cat, year)]
		failCh = GJmodel['ptbin%d%sfail%s' % (ptbin, cat, year)]
                ...
                passObs = passCh.getObservation()
                    ...
                    #GJparams = np.array([rl.IndependentParameter('GJparam_ptbin%dmjjbin%d%s%s_%d' % (ptbin, mjjbin, cat, year, i), 0) for i in range(msd.nbins)])
...
#                    fail_GJ = rl.ParametericSample('ptbin%dmjjbin%d%sfail%s_GJ' % (ptbin, mjjbin, cat, year), rl.Sample.BACKGROUND, msd, scaledparams[0])
...
#                    pass_GJ = rl.TransferFactorSample('ptbin%dmjjbin%d%spass%s_GJ' % (ptbin, mjjbin, cat, year), rl.Sample.BACKGROUND, tf_MCtempl_params[ptbin, :], fail_GJ)
...
#                                    sample.setParamEffect(sys_dict[sys], eff_up, eff_do)
 ...
 #                                   sample.setParamEffect(sys_dict[sys], eff_up, eff_do)
                            ...
                            # GJ scale and PDF uncertainties on Higgs signal    
...
#                data_obs = get_template('data', isPass, binindex+1, cat+'_', obs=msd, syst='nominal')
                data_obs = get_template('Data', isPass, ptbin+2, obs=msd, syst='nominal')
...
#                fail_Data = rl.ParametericSample('ptbin%dmjjbin%d%sfail%s_Data' % (ptbin, mjjbin, cat, year), rl.Sample.BACKGROUND, msd, scaledparams)
...
#                pass_Data = rl.TransferFactorSample('ptbin%dmjjbin%d%spass%s_Data' % (ptbin, mjjbin, cat, year), rl.Sample.BACKGROUND, tf_params[cat][ptbin, :], fail_Data)
...
#                    tqqfail.setParamEffect(tqqnormSF, 1 * tqqnormSF)
    ...
    kfactor_Data = 1.0*Nfail_data/Nfail_Data_MC
    print(kfactor_Data)

    # Fill in muon CR
#    if do_muon_CR:
#        templates = {}
#        samps = ['GJ','Wjets','Zjets','Zjetsbb','EWKW','EWKZ','EWKZbb','ttbar','singlet','VV']
#        for region in ['pass', 'fail']:
