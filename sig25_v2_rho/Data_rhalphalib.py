...
#        name += 'pt'+str(ptbin)+'_'+sName+'_'+syst
 #   else:
  #      name += sName+'_'+syst
    ...
    name = sName+'_'+name+'_'+"jet_pt_soft"
...
def plot_mctf(tf_MCtempl, msdbins):
    ...
    rhopts_scaled = (rhopts - (-7.25)) / ((1.0) - (-7.25))
    ...
    mpt_TF = TH2F("mpt_TF", "Mass vs. pT Transfer Factor", 40, 0, 200, 40, 0, 2000)
...
#	print(mpt[i][2])
    ...
    cb.set_label("Ratio")
    fig.savefig("MCTF_msdpt_2018_DataSUB.png",bbox="tight")
    plt.clf()

    # arrays for plotting pt vs rho                                          
    rhos = np.linspace(-7.25,-1.0,51)
    ...
    rpt_TF = TH2F("rpt_TF", "Rho vs. pT Transfer Factor", 50, -7.25, -1, 40, 0, 2000)
    ...
    cb = fig.colorbar(h[3],ax=ax)
    cb.set_label("Ratio")
    fig.savefig("MCTF_rhopt_2018_DataSUB.png",bbox="tight")
...
#        SF = json.load(f)
    ...
    rhopts = 2*np.log(msdpts/ptpts)
    ptscaled = (ptpts - 100.0) / (600.0 - 100.0)
    rhoscaled = (rhopts - (-7.25)) / ((-1.0) - (-7.25))
    ...
    for ptbin in range(npt):
        ...
        failTempl = get_template('Data', 0, ptbin+3, obs=msd, syst='nominal') 
        passTempl = get_template('Data', 1, ptbin+3, obs=msd, syst='nominal') 
        
	TTfailTempl = get_template('TTBar', 0, ptbin+3, obs=msd, syst='nominal') 
        TTpassTempl = get_template('TTBar', 1, ptbin+3, obs=msd, syst='nominal') 
	...
	if np.any(failObs < 0):
	    raise ValueError('failing data negative for some bins...', failObs)
	if np.any(passObs < 0):
	    raise ValueError('passing data negative for some bins...', passObs)
    ...
    #New Stuff
    # Redefine bins    
    ptbins = np.linspace(100, 2000, 39)
    npt = len(ptbins) - 1
    msdbins = np.linspace(0, 200, 41)
    msd = rl.Observable('msd', msdbins)
    ...
    ptpts, msdpts = np.meshgrid(ptbins[:-1] + 0.5 * np.diff(ptbins), msdbins[:-1] + 0.5 * np.diff(msdbins), indexing='ij')
    rhopts = 2*np.log(msdpts/ptpts)
    ptscaled = (ptpts - 100.0) / (2000.0 - 100.0)
    rhoscaled = (rhopts - (-7.25)) / ((-1.0) - (-7.25))
    validbins = (rhoscaled >= 0) & (rhoscaled <= 1)
    rhoscaled[~validbins] = 1  # we will mask these out later
...
#            mask = validbins[ptbin]
            ...
            for sName in samps:
                ...
                templates[sName] = get_template2(sName, isPass, ptbin+3, obs=msd, syst='nominal')
            ...
            data_obs = get_template2('Data', isPass, ptbin+3, obs=msd, syst='nominal')
...
#	print("Numpy Dimensions"+str(initial_Data.ndim))
        if np.any(initial_Data < 0.):
	    for i in range(np.size(initial_Data)):
#		print(initial_Data[i])
		if initial_Data[i] < 0:
	#	    print(initial_Data[i])
		    print("ptbin"+str(ptbin)+"msdbin"+str(i)+" is negative")
		    initial_Data[i] = 0
		 #   initial_Data[1][i] = 0
