...
def plot_mctf(tf_MCtempl, msdbins):
    ...
    rhopts_scaled = (rhopts - (-8.0)) / ((-1.0) - (-8.))
...
#	print(mpt[i][2])
    ...
    rpt_TF = TH2F("rpt_TF", "Rho vs. pT Transfer Factor", 56, -8, -1, 40, 0, 2000)
...
#        SF = json.load(f)
    ...
    S75effSF = rl.IndependentParameter('S75effSF_{}'.format(year), 1., 0, 10)
    S75normSF = rl.IndependentParameter('S75normSF_{}'.format(year), 1., 0, 10)
    S100effSF = rl.IndependentParameter('S100effSF_{}'.format(year), 1., 0, 10)
    S100normSF = rl.IndependentParameter('S100normSF_{}'.format(year), 1., 0, 10)
...
#    msdbins = np.linspace(0, 100, 21)
	...
	#Placeholder WGamma and ZGamma Added here
	S75failTempl = get_template('Sig75', 0, ptbin+1, obs=msd, syst='nominal') 
        S75passTempl = get_template('Sig75', 1, ptbin+1, obs=msd, syst='nominal') 
	S100failTempl = get_template('Sig100', 0, ptbin+1, obs=msd, syst='nominal') 
        S100passTempl = get_template('Sig100', 1, ptbin+1, obs=msd, syst='nominal') 
	...
	S75failSamp = rl.TemplateSample(failCh.name+'_'+"S75", rl.Sample.BACKGROUND, S75failTempl)
	S75passSamp = rl.TemplateSample(passCh.name+'_'+"S75", rl.Sample.BACKGROUND, S75passTempl)
	S100failSamp = rl.TemplateSample(failCh.name+'_'+"S100", rl.Sample.BACKGROUND, S100failTempl)
	S100passSamp = rl.TemplateSample(passCh.name+'_'+"S100", rl.Sample.BACKGROUND, S100passTempl)
	...
	failCh.addSample(S75failSamp)
	passCh.addSample(S75passSamp)
	failCh.addSample(S100failSamp)
	passCh.addSample(S100passSamp)
	failCh.addSample(TTfailSamp)
	passCh.addSample(TTpassSamp)

        Datafail += sum([val for val in failCh.getObservation()[0]]) - sum([val for val in TTfailSamp.getExpectation(nominal=True)]) - sum([val for val in S75failSamp.getExpectation(nominal=True)])- sum([val for val in S100failSamp.getExpectation(nominal=True)])
        fail_count += sum([val for val in failCh.getObservation()[0]])

	print(Datafail)
	print(fail_count)
        Datapass += sum([val for val in passCh.getObservation()[0]]) - sum([val for val in TTpassSamp.getExpectation(nominal=True)]) - sum([val for val in S75passSamp.getExpectation(nominal=True)]) - sum([val for val in S100passSamp.getExpectation(nominal=True)])
	...
	failObs -= S75failSamp.getExpectation(nominal=True)
	passObs -= S75passSamp.getExpectation(nominal=True)
	failObs -= S100failSamp.getExpectation(nominal=True)
	passObs -= S100passSamp.getExpectation(nominal=True)
...
#    sigs = ['ggF','VBF','WH','ZH','ttH']
    samps = ['TTBar','Sig25', 'Sig75', 'Sig100']
    sigs = ['Sig25']
    for ptbin in range(npt):
...
#            raise ValueError('initial_Data negative for some bins..', initial_Data)
        ...
        S75pass = passCh['Sig75']
        S75fail = failCh['Sig75']
        S75PF = S75pass.getExpectation(nominal=True).sum() / S75fail.getExpectation(nominal=True).sum()
        S75pass.setParamEffect(S75effSF, 1*S75effSF)
        S75fail.setParamEffect(S75effSF, (1 - S75effSF) * S75PF + 1)
        S75pass.setParamEffect(S75normSF, 1*S75normSF)
        S75fail.setParamEffect(S75normSF, 1*S75normSF)
        S100pass = passCh['Sig100']
        S100fail = failCh['Sig100']
        S100PF = S100pass.getExpectation(nominal=True).sum() / S100fail.getExpectation(nominal=True).sum()
        S100pass.setParamEffect(S100effSF, 1*S100effSF)
        S100fail.setParamEffect(S100effSF, (1 - S100effSF) * S100PF + 1)
        S100pass.setParamEffect(S100normSF, 1*S100normSF)
        S100fail.setParamEffect(S100normSF, 1*S100normSF)
