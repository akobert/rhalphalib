...
def plot_mctf(tf_MCtempl, msdbins):
    ...
    pts = np.array([200,250,300,400,500,700,900,1200,1500,2000])
...
#    ZGnormSF = rl.IndependentParameter('ZGnormSF_{}'.format(year), 1., 0, 10)
    ...
    # define bins    
    ptbins = np.array([200,250,300,400,500])
...
#    msdbins = np.linspace(0, 50, 11)
    ...
    for ptbin in range(npt):
        ...
        failTempl = get_template('GJ', 0, ptbin+7, obs=msd, syst='nominal') 
        passTempl = get_template('GJ', 1, ptbin+7, obs=msd, syst='nominal') 
...
#    sigs = ['ggF','VBF','WH','ZH','ttH']
    ...
    for ptbin in range(npt):
		...
		print("Sample "+region+" "+sName)
                templates[sName] = get_template2(sName, isPass, ptbin+7, obs=msd, syst='nominal')
		...
		print(sName+" "+region+" Expectation: "+str(sample.getExpectation(nominal=True).sum()))
            ...
            data_obs = get_template2('Data', isPass, ptbin+7, obs=msd, syst='nominal')
...
#            raise ValueError('initial_Data negative for some bins..', initial_Data)
        ...
        WGfail.setParamEffect(WGeffSF, 1.1)
        WGpass.setParamEffect(WGnormSF, 1.1)
        WGfail.setParamEffect(WGnormSF, 1.1)
        ...
        ZGfail.setParamEffect(ZGeffSF, 1.1)
        ZGpass.setParamEffect(ZGnormSF, 1.1)
        ZGfail.setParamEffect(ZGnormSF, 1.1)
	...
	#print("Top Fail Expectation: "+str(tqqfail.getExpectation(nominal=True).sum()))
        ...
        tqqfail.setParamEffect(tqqeffSF, 1.1)
        tqqpass.setParamEffect(tqqnormSF, 1.1)
        tqqfail.setParamEffect(tqqnormSF, 1.1)
