...
def plot_mctf(tf_MCtempl, msdbins):
    ...
    mpt_TF = TH2F("mpt_TF", "pT vs. Mass Transfer Factor", 9, widebins, 40, 0, 200)
    ...
    cb.set_label("Ratio")
    fig.savefig("MCTF_msdpt_2018_sig25_corr_veryhigh.png",bbox="tight")
    plt.clf()
    ...
    cb = fig.colorbar(h[3],ax=ax)
    cb.set_label("Ratio")
    fig.savefig("MCTF_rhopt_2018_sig25_corr_veryhigh.png",bbox="tight")
...
#    msdbins = np.linspace(0, 50, 11)
    ...
    tf_MCtempl = rl.BernsteinPoly('tf_MCtempl', (5, 5), ['pt', 'rho'], init_params=initial_vals, limits=(-200, 200))
    ...
    tf_dataResidual = rl.BernsteinPoly('tf_dataResidual', (5, 5), ['pt', 'rho'], limits=(-200, 200))
...
#	print(fail_Data.type())

#	for i in range(np.size(fail_Data)):
#		print("Fail Data pT bin# "+str(ptbin)+" msd bin# "+str(i)+": "+str(fail_Data(.605263, .1)))
#	for i in range(np.size(pass_Data)):
#		print("Pass Data pT bin# "+str(ptbin)+" msd bin# "+str(i)+": "+str(pass_Data[i]))
