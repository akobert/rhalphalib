...
def drawDiagnostic(name, ifile1):
	...
	sig_name = "sig25_v2"
	ptBins = [120,135,155,175,200,250,300,400,500]
	mBins = MakeNBinsFromMinToMax(40,0,200)
	VAR = ["jpt", ptBins, "Jet pT (GeV)", "sdm", mBins, "Softdrop Mass (GeV)"]

	Blind = True
	LUMI = 1.404 #Temporary luminosity with 10% of 2018 data B and C
	...
	for pf in ["pass", "fail"]:
		for a in range(0, 8):
			...
			cDATA_2D = refF.Get("Data_pass_jet_pt_soft_wide3")
			cDATA = cDATA_2D.ProjectionY("proj"+str(a), a+2, a+2)
...
#					cDATA = CbnF.Get("shapes_"+P+"/"+NAME+"/data")
				...
				#	cTT = cTT.Clone()
					...
					cSIG = CbnF.Get("shapes_"+P+"/"+NAME+"/total_signal")
                                        cS75 = CbnF.Get("shapes_"+P+"/"+NAME+"/Sig75")
                                        cS100 = CbnF.Get("shapes_"+P+"/"+NAME+"/Sig100")
...
#					cDATA = convertAsymGraph(cDATA, cTT, "data"+P)
					Hvec = []
					for i in [cDATA, cTT, cTBKG, cSIG, cS75, cS100]:
...
#					C2e.Print("results/"+NAME+"/Sig2D_"+P+".png")
					...
					#	Err = bkg.GetBinError(i)
						Err = math.sqrt(bkg.GetBinContent(i))	
					...
					#	ue = min(5.0, ue)
						...
						tPBox = ROOT.TBox(blX, blY, trX, trY)
					...
					L.AddEntry(ttbar, "t#bar{t} component", "L")
                                        L.AddEntry(s75, "75 GeV Signal", "L")
                                        L.AddEntry(s100, "100 GeV Signal", "L")
					...
					pull.Draw("esame")
					p22.RedrawAxis()
					C.Print("results/"+NAME+"/"+savename+"_"+pf+str(a)+"_errScale.root")
					C.Print("results/"+NAME+"/"+savename+"_"+pf+str(a)+"_errScale.png")
